...
<div class="container" style="max-width: 1400px; margin: 0 auto; padding: 3rem 2rem;">
    ...
    <header class="archive-header" style="text-align: center; margin-bottom: 3rem;">
        ...
        <p class="archive-description" style="font-size: 1.2rem; color: rgba(255, 255, 255, 0.8); max-width: 800px; margin: 0 auto;">
            Scopri i nostri servizi di ricerca AI e soluzioni avanzate
        </p>
    </header>
            ...
            <?php while (have_posts()) : the_post(); ?>
                <article id="post-<?php the_ID(); ?>" <?php post_class('service-card'); ?>>
                    <?php if (has_post_thumbnail()) : ?>
                        <a href="<?php the_permalink(); ?>" class="service-thumbnail" style="display: block; margin-bottom: 1.5rem; border-radius: 12px; overflow: hidden;">
                            <?php the_post_thumbnail('medium', array('style' => 'width: 100%; height: 200px; object-fit: cover;')); ?>
                        </a>
                    <?php endif; ?>
                    
                    <header class="service-header">
                        <h2 class="service-title" style="margin-bottom: 0.5rem;">
                            <a href="<?php the_permalink(); ?>" style="color: #00d9ff; text-decoration: none; font-size: 1.5rem;">
                                <?php the_title(); ?>
                            </a>
                        </h2>
                        
                        <div class="service-meta" style="color: rgba(255, 255, 255, 0.6); font-size: 0.9rem; margin-bottom: 1rem;">
                    ...
                    <div class="service-excerpt" style="color: rgba(255, 255, 255, 0.8); line-height: 1.6; margin-bottom: 1rem;">
                        <?php the_excerpt(); ?>
                    </div>
    ...
    <?php else : ?>
        <div class="no-results" style="text-align: center; padding: 4rem 2rem;">
            <h2 style="color: rgba(255, 255, 255, 0.8); font-size: 1.5rem;">Nessun servizio trovato</h2>
            <p style="color: rgba(255, 255, 255, 0.6);">Al momento non ci sono servizi disponibili.</p>
        </div>
    <?php endif; ?>
